...
try:
    ...
    src = pd.DataFrame(columns=["User Question", "Normalized Query", "AI Pipeline"])

# Only process rows that have a pipeline
validRows = src[src["AI Pipeline"].notna()].copy()
validRows = validRows[["User Question", "Normalized Query", "AI Pipeline"]].reset_index(drop=True)
validRows.index += 1

# ── Build working dataframe ───────────────────────────────────────────────────
df = validRows.copy()
for col in ["AI isValid", "AI Refinement", "AI Context", "Passed"]:
...
def callAgent(item):
    ...
    try:
        ...
        return i, None, None, None, f"Pipeline parse error: {e}"

    payload = {
        "agent": "mongo_query_validator",
    ...
    try:
        ...
        if not response.ok:
            return i, None, None, None, f"HTTP {response.status_code}: {response.text[:200]}"
        result = response.json()
        return (
            i,
            result.get("isValid"),
            result.get("refinement"),
            result.get("context"),
            None,
        )
    except requests.exceptions.JSONDecodeError as e:
        return i, None, None, None, f"JSON error: {e} | body: {response.text[:200]}"
    except Exception as e:
...
for batchStart in range(0, len(rows), BATCH_SIZE):
    ...
    with ThreadPoolExecutor(max_workers=BATCH_SIZE) as executor:
        ...
        for future in as_completed(futures):
            i, isValid, refinement, context, error = future.result()
            if error:
                print(f"✗ {i}: Failed — {error}")
            else:
                df.at[i, "AI isValid"]   = isValid
                df.at[i, "AI Refinement"] = refinement
                df.at[i, "AI Context"]    = context
...
with pd.ExcelWriter(EXCEL_FILE, engine="openpyxl", mode=writeMode, if_sheet_exists="replace") as writer:
    ...
    AI_COLS = {"AI isValid", "AI Refinement", "AI Context"}
...
print("Done. Query Validator sheet populated.")
